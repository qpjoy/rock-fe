...
export const ProductSpecificationsAccordion = ({ type, contentRef }: any) => {
        ...
        return (
          <IonAccordion key={`accordion_${header}_${index}`}>
            ...
            <IonList slot="content" className="ion-no-padding">
              {options.map((option: any, index2: any) => {
                const { label, value } = option;

                return (
                  <IonItem
                    key={`accordion_${header}_${option}_${index2}`}
                  ...
                  >
                    ...
                    <IonLabel className={wrapText && "ion-text-wrap"}>
                      <IonNote
                        color={
                          noteColor ? (value ? "success" : "danger") : "medium"
                        }
                      >
                        {noteColor
                          ? value
